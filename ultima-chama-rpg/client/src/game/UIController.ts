...
export class UIController {
  ...
  constructor(private readonly callbacks: UICallbacks) {
    ...
    this.root.innerHTML = `
      ...
      <section class="parchment-panel pause-menu" id="pause-panel" aria-hidden="true">
        ...
        <nav class="pause-tabs" aria-label="Categorias de configuração" role="tablist"><button class="tab-active" data-pause-tab="game" role="tab" aria-selected="true">JOGO</button><button data-pause-tab="audio" role="tab" aria-selected="false">ÁUDIO</button><button data-pause-tab="accessibility" role="tab" aria-selected="false">ACESSO</button></nav><footer class="pause-restore"><button id="restore-settings" type="button">↺ Restaurar padrões de todas as abas</button><small>Jogo, áudio e acessibilidade voltam à configuração inicial.</small></footer>
      ...
      <section class="settings-confirmation" id="restore-confirmation" aria-hidden="true" aria-modal="true" aria-labelledby="restore-confirmation-title" role="dialog"><div class="settings-confirmation-card"><span>CONFIGURAÇÃO LOCAL</span><h3 id="restore-confirmation-title">Restaurar todos os padrões?</h3><p>Idioma, dificuldade, volumes, narração e legendas voltarão às escolhas iniciais da crônica.</p><div><button id="cancel-restore" type="button">Cancelar</button><button id="confirm-restore" type="button">Restaurar padrões</button></div></div></section>
  ...
  private bind() {
    ...
    this.root.querySelector<HTMLButtonElement>("#restore-settings")?.addEventListener("click", () => this.openRestoreConfirmation());
    this.root.querySelector<HTMLButtonElement>("#cancel-restore")?.addEventListener("click", () => this.closeRestoreConfirmation());
    this.root.querySelector<HTMLButtonElement>("#confirm-restore")?.addEventListener("click", () => { this.closeRestoreConfirmation(); this.callbacks.resetSettings(); });
    this.root.addEventListener("keydown", (event) => { if (event.key === "Escape" && this.element("restore-confirmation")?.getAttribute("aria-hidden") === "false") this.closeRestoreConfirmation(); });
  ...
  openRestoreConfirmation() { const modal = this.element("restore-confirmation"); if (!modal) return; modal.setAttribute("aria-hidden", "false"); window.setTimeout(() => this.element<HTMLButtonElement>("cancel-restore")?.focus(), 0); }
