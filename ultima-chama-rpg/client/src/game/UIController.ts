  ...
  constructor(private readonly callbacks: UICallbacks) {
    ...
    this.root.innerHTML = `
      ...
      <section class="parchment-panel pause-menu" id="pause-panel" aria-hidden="true">
        ...
        <p data-i18n="localSave">O jogo é salvo localmente neste navegador.</p><section class="accessibility-settings"><header><span>APRESENTAÇÃO E ACESSIBILIDADE</span><b>CONFIGURAÇÃO LOCAL</b></header><label><input id="narration-enabled" type="checkbox" checked/> <span>Narração natural</span><small>Reproduz somente arquivos de voz finalizados; não usa leitura robótica.</small></label><label><input id="dialogue-subtitles-enabled" type="checkbox" checked/> <span>Legendas de diálogo</span><small>Exibe e fecha cada fala ao fim do áudio ou da leitura.</small></label><label><input id="action-subtitles-enabled" type="checkbox" checked/> <span>Legendas de ações e alertas</span><small>Mostra técnicas, compras e notificações breves de combate.</small></label></section><section class="voice-settings"><header><span data-i18n="voices">VOZES DE CENA · PT-BR</span><b data-i18n="replaceable">SUBSTITUÍVEIS</b></header><p>As falas são narradas em PT-BR por arquivos naturais. Use as prévias para ouvir as assinaturas de voz; arquivos futuros podem substituir estes perfis.</p><div><button data-preview-voice="Kael">Kael</button><button data-preview-voice="Dheren Varenn">Dheren Varenn</button><button data-preview-voice="Lyra">Lyra</button><button data-preview-voice="Mira">Mira</button></div></section>
      </section>
